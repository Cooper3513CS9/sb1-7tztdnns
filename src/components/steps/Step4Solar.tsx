...
import { solarPanelGroups } from '../../config/formOptions';

export default function Step4Solar() {
  const { userData, updateUserData, colors } = useUserData();
  const { solar } = userData;
  
  return (
    <Card title="Stap 5: Uw Zonnepanelen" icon={<Zap size={28} style={{color: colors.primary}}/>}>
      <p className="text-slate-600 mb-6">Geef ons informatie over uw zonnepaneleninstallatie. Als u de exacte gegevens niet weet, kunt u een schatting kiezen.</p>
      ...
      {solar.knowsExactSolarData ? (
        <>
          ...
          <InputField
            ...
            unit="kWh"
            helpText="U vindt dit meestal op de omvormer of in uw monitoring app."
            min={0}
          />
        </>
      ) : (
      ...
      <StepNavigation prevStep="intake-step4" nextStep="intake-step6" />
      <StepProgress step={5} stepTitle="Zonnepanelen" /> 
    </Card>
  );
}